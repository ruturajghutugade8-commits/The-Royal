...
def add_to_cart(product_id):
    product = query_db("SELECT * FROM products WHERE id=%s", (product_id,), fetch=True)
    if not product:
...
def cart():
    ...
    for product_id, quantity in cart_data.items():
        products = query_db("SELECT * FROM products WHERE id=%s", (int(product_id),), fetch=True)
        if products:
            product = products[0]
...
def admin_dashboard():
    ...
    stats = {
        "users": query_db("SELECT COUNT(*) AS total FROM users", fetch=True)[0]["total"],
        "products": query_db("SELECT COUNT(*) AS total FROM products", fetch=True)[0]["total"],
        "orders": query_db("SELECT COUNT(*) AS total FROM orders", fetch=True)[0]["total"],
        "revenue": query_db(
            "SELECT COALESCE(SUM(total_amount),0) AS total FROM orders "
            "WHERE status NOT IN ('Cancelled')", fetch=True
        )[0]["total"]
    }
...
def edit_product(product_id):
    ...
    products = query_db("SELECT * FROM products WHERE id=%s", (product_id,), fetch=True)
    if not products:
        flash("Product not found.")
        return redirect(url_for("admin_products"))
    ...
    return render_template("product_form.html", product=products[0])
...
def admin_order_detail(order_id):
    ...
    return render_template("admin_order_detail.html", order=orders[0], items=items)
...
def admin_update_order_status(order_id):
    ...
    query_db("UPDATE orders SET status=%s WHERE id=%s", (status, order_id))
